...
class Zomato:
    ...
    def restaurant_search(self, offset, query="", latitude="", longitude="", cuisines="", limit=5):
        """
        Takes either query, latitude and longitude or cuisine as input.
        Returns a list of Restaurant IDs.
        """
        cuisines = "%2C".join(cuisines.split(","))
        print("restaurant_search cuisines:", cuisines)
        if str(limit).isalpha() == True:
            raise ValueError('LimitNotInteger')
        headers = {'Accept': 'application/json', 'user-key': self.user_key}
        print(base_url + "search?q=" + str(query) + "&count=" + str(limit) + "&start=" + str(offset) + "&sort=rating" + "&order=desc" + "&lat=" + str(latitude) + "&lon=" + str(longitude) + "&cuisines=" + str(cuisines))
        r = (requests.get(base_url + "search?q=" + str(query) + "&count=" + str(limit) + "&start=" + str(offset) + "&sort=rating" + "&order=desc" + "&lat=" + str(latitude) + "&lon=" + str(longitude) + "&cuisines=" + str(cuisines), headers=headers).content).decode("utf-8")
